...
3. Khởi chạy giao diện Frontend:
   ...
   npm run dev
   ```
4. Truy cập vào đường link hiển thị trên Terminal (thường là `http://localhost:5173`) bằng trình duyệt Chrome/Edge.
...
Lần đầu tiên chạy project, Database của bạn sẽ trống rỗng. Hãy sử dụng Postman hoặc trình duyệt gọi API Seeding (nếu có) để tạo tài khoản Admin mặc định hoặc dữ liệu mẫu. 
*(Nếu dự án chưa cấu hình lệnh seed tự động, bạn cần tạo tài khoản trực tiếp qua Database hoặc Postman qua route `/auth/register`)*.
...
5. **Quản lý nhân sự:** Truy cập `/staff` để cấp tài khoản cho nhân viên mới hoặc xóa nhân viên nghỉ việc.
